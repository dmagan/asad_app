...
const VIPChannel = ({ isDarkMode, isOpen = true, onClose }) => {
...
const fetchPosts = async (pageNumber) => {
  try {
    ...
    const response = await fetch(
      `https://siwoxelo.myhostpoint.ch/wp-json/wp/v2/posts?_embed&order=desc&orderby=date&per_page=10&page=${pageNumber}&categories=109`,
      {
        headers: {
...
useEffect(() => {
  ...
  return () => {
    ...
    if (posts.length > 0) {
      ...
      vipNotificationService.markAllAsRead(latestPostId);
    } else {
     vipNotificationService.markAllAsRead();
    }
  };
